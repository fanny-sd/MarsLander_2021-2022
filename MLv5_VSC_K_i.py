...
def simulate(X0, V0, land, landing_site, 
    ...
    for i in range(Nstep):
        ...
        if parachute == 0:
            ...
            drag[i, :] = -Dc*np.linalg.norm(V)*V           # see board explanation #old: Dc*(V[1]**2) 
...
def proportional_autopilot(i, X, V, fuel, rotate, power,parameters, parachute, dt, e_prev, Nstep):
    K_v,K_p,K_h,K_i = parameters
    
    #K_i= 0; # passed
    K_d = 0; # pass them in paramemeters
    ...
    v_target_horz = abs(c_h+K_h*dist) # was: -K_h*dist; from test, new one seems to work better (fuel lasted longer)
    ...
    power = min(max(Pout, 0.0), 20000.0)   # max thrust
    ...
    if i % 100 == 0:
        print(f'e={e:8.3f} Pout={Pout:8.3f} power={power:8.3f} K_p={K_p:8.3f} K_h={K_h:8.3f} K_v={K_v:8.3f} K_i={K_i:8.3f}')     
...
K_v_list = np.linspace(0.0005,0.5,4)
K_h_list = np.linspace(0.0005,0.5,4)   
K_p_list = np.linspace(5,500,5)  
K_i_list = np.linspace(0.0005,10,10)
...
for i, K_v in enumerate(K_v_list):
    for j, K_p in enumerate(K_p_list):
        for k, K_h in enumerate(K_h_list):
            for f, K_i in enumerate(K_i_list):
                ...
                try:
                    Xs, Vs, thrust, success,fuel_warning_printed, fuel, rotate, parachute = simulate(X0, V0, land, landing_site, dt=0.1, Nstep=3500, 
                                                    autopilot=proportional_autopilot, fuel=400,parameters=[K_v,K_p,K_h,K_i],parachute=None)
                except IndexError:
...
print('Number of other trials that ran:',trials)  
