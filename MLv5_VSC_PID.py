...
def simulate(X0, V0, land, landing_site, 
    ...
    for i in range(Nstep):
        ...
        if autopilot is not None:
            ...
            assert 0 <= power <= 12000 #changed from 4000, then 12 000
...
#K_v_list = np.linspace(0.0005,0.01,3)
#K_h_list = np.linspace(0.0005,0.01,3)   
K_p_list = np.linspace(0.005,50,5) 
K_i_list = np.linspace(0.005,100,5)
K_d_list = np.linspace(0.005,100,5)

K_v_list = [0.001,0.005,0.01]
K_h_list = [0.001,0.005,0.01] 
    ...
    for j, K_p in enumerate(K_p_list):
        for k, K_h in enumerate(K_h_list):
            for f, K_i in enumerate(K_i_list):
                for f, K_d in enumerate(K_d_list):
